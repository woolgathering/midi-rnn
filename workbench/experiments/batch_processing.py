...
paths = glob('midi/*.mid') # read the filenames from a directory into an array
...
output_dir = 'midi/data/parsed/' # output directory
trans = [0,0,0,0,0,0,0,0,0] # this MUST be the same size as paths!! Each number is the number of semitones to transpose by
...
for i, path in enumerate(paths):
  try:
    ...
    parsed = simple_parse_midi(midifile, trackNum=0, tempo=get_tempo(midifile), transpose=trans[i])
