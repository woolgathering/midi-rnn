...
def make_and_train(data, memory=10, checkpoint_path='/home/jlsundst/'):
  raw_data = []
  ini = 0
  end = ini + memory
  while end < len(data):
  ...
  checkpoint = ModelCheckpoint(checkpoint_path, verbose=1, save_best_only=True, mode='max')
  callbacks_list = [checkpoint]

  print ("Starting training...")
  with tf.device('/gpu:0'):
    #model.fit(X, y, epochs=500, batch_size=128, callbacks=callbacks_list)
    model.fit(X, y, validation_split=0.2, epochs=500, batch_size=32, callbacks=callbacks_list)
